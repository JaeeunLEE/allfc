    ...
    <ul>
     ...
     <%
      ...
      if(admin1DAO.count("act","M") == 0){
        out.println("최초 등록 계정이므로 Master 계정입니다.");
        vo.setAct("M"); //최고 관리자 지정
        vo.setConfirm("Y"); //최고 관리자이므로 메일 확인 처리
      }else{
        vo.setAct("H"); //관리자 승인 필요
        vo.setConfirm("N"); //메일 확인 안됐음
      }
...
content += "http://172.16.7.13:9090/admin_v1jq/admin1/confirm.jsp?email=" + email + "&auth=" + auth;
...
try {
  ...
  out.println("<u>인증 메일이 발송되었습니다.</u><br><br>");
