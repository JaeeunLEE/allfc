...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
   ...
   * @param col 검색 컬럼  
   * @param word 검색어
   * @param list_file 페이징에 사용할 파일명
   * @return 페이징 생성 문자열
   */ 
  public String paging4(
      int totalRecord, 
      int nowPage, 
      int recordPerPage, 
      String col, 
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='./"+list_file+"?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>이전</A></span>"); 
    } 
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ 
        ...
        str.append("<span class='span_box_1'><A href='./"+list_file+"?col="+col+"&word="+word+"&nowPage="+i+"'>"+i+"</A></span>");   
      } 
    } 
     
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='./"+list_file+"?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
